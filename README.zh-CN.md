...
- `x-twitter-status`：限定在 X/Twitter 当前 status 对话，排除推荐内容和无关时间线推文。
...
抽取器实现规范见 [EXTRACTORS.md](./EXTRACTORS.md)。JSON rules 放在 `src/extractors/rules/`；TypeScript extractors 放在 `src/extractors/plugins/`，需要语义边界、多媒体作用域或 SPA 逻辑时使用，并统一在 `src/extractors/manifest.ts` 登记。
